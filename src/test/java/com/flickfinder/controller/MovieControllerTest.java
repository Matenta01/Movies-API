...
class MovieControllerTest {
	...
	void testGetAllMovies() {
		...
		try {
			verify(movieDAO).getAllMovies(limit);
	...
	void testThrows500ExceptionWhenGetAllDatabaseError() throws SQLException {
		when(movieDAO.getAllMovies(limit)).thenThrow(new SQLException());
		movieController.getAllMovies(ctx);
	...
	void testGetRatingsByYear() {
		...
		try {
			verify(movieDAO).getAllMoviesInSpecificYear(2008, 1000, 50);
		} catch (SQLException e) {
